...
def constructCommand(config, testName):
    ...
    if command == HADOOP and config.get(testName, JAR_CLASS) \
        == 'TestDFSIO':
        process = getTestDfsioArguments(config, testName)
    elif command == HADOOP and config.get(testName, JAR_CLASS) \
        == 'Terasort':
...
def copyHadoopConfigFiles(targetDir):
    configFiles = ['/usr/local/hadoop/etc/hadoop/core-site.xml',
...
def main():
    ...
    testRunId = configFileName + time.strftime('%Y%m%dT%H%M%S',
            time.localtime())
    ...
    outfile = createOpenFile(logFileName,
                             FILE_FLAG_CREATE_IF_NOT_EXISTS)
    resultsFile = createOpenFile(resultFileName,
                                 FILE_FLAG_CREATE_IF_NOT_EXISTS)
    ...
    for testName in testNames:
        try:
            ...
            if command == HADOOP and testRunConfig.get(testName,
                    JAR_CLASS) == 'TestDFSIO':
                extractedResults = extractDFSIOResults(lines)
            elif command == HADOOP and testRunConfig.get(testName,
                    JAR_CLASS) == 'Terasort':
            ...
            testResult[PARAMS] = getTestRunParams(testRunConfig,
                    testName)
